...
export default function Libros() {
  ...
  const openEdit = (libro) => {
    ...
    setForm({ titulo: libro.titulo, autor: libro.autor, categoria: libro.categoria, anio: String(libro.anio), stock: String(libro.stock || 1) })
    setFormErrors({})
    setModalOpen(true)
  }
  ...
  const handleDelete = async () => {
    try {
      ...
      setLibros(libros.filter((l) => l._id !== deleteId))
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingLibro) {
        await updateBook(editingLibro._id, payload)
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => confirmDelete(row._id)} className="p-1.5 rounded-lg hover:bg-peligro-100 text-peligro-600 transition-colors cursor-pointer" title="Eliminar">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              {formErrors.stock && <p className="text-xs text-peligro-600 mt-1">{formErrors.stock}</p>}
            </div>
          </div>
